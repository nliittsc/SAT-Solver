...
main = hspec $ do
  describe "toBin" $ do
    prop "converts to binary and back" $ do
      forAll (choose (0,1000) :: Gen Int) prop_bin_conv

  describe "genClause" $ do
    prop "generates only valid clauses" $
      forAll (genClause 1000) prop_valid_clause

  describe "dpllSolver" $ do
    prop "solver matches brute search" $
      forAll (genThreeSAT 20 50) prop_dpll_correct